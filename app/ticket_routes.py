...
# # ────────────────────────────────────────────
# # Add Comment
# @ticket_bp.route("/ticket/comment/<int:ticket_id>", methods=["POST"])
...
# # ────────────────────────────────────────────
# # Tag Ticket
# @ticket_bp.route("/ticket/tag/<int:ticket_id>", methods=["POST"])
# def add_ticket_tags(ticket_id):
#     ticket = Ticket.query.get(ticket_id)
#     if not ticket:
#         return jsonify({"error": "Ticket not found"}), 404

#     data = request.json
#     user_ids = data.get("user_ids")  # e.g. [12, 15, 20]
#     assigner_id = data.get("user_id")  # who is tagging
...
#             # ─── Send email in background ───
#             user_info = get_user_info_by_id(uid)
#             assigner_info = get_user_info_by_id(assigner_id)

#             if user_info:
#                 assigner_name = assigner_info["username"] if assigner_info else f"User {assigner_id}"
#                 threading.Thread(
#                     target=run_async_email,
#                     args=(send_email(
#                         to=user_info["email"],
#                         subject=f"You were tagged in Ticket #{ticket.id}",
#                         body=f"Hello {user_info['username']},\n\n"
#                              f"You were tagged in ticket '{ticket.title}' by {assigner_name}."
#                     ),)
#                 ).start()

#     return jsonify({
#         "success": True,
#         "message": f"Tags added for users: {added_tags}",
#         "tags": added_tags
#     })
